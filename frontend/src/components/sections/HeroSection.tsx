  ...
  return (
    <section id="home" className="min-h-[100dvh] flex flex-col items-center justify-between overflow-x-hidden" style={{ paddingBottom: '2.5rem' }}>
      ...
      <div className="w-full max-w-7xl mx-auto flex justify-center pt-24 md:pt-28 lg:pt-32 pb-4">
        <ExpressLogo iconSize="clamp(2rem, 8vw, 3.5rem)" textSize="clamp(2rem, 8vw, 3.5rem)" />
      </div>

      {/* Middle: Text + Image - grows to fill remaining space */}
      <div className="flex-1 flex items-center w-full">
        <div className="max-w-7xl mx-auto w-full px-4 flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-24">
          
          {/* Hero Imagery (Moved up on mobile) */}
          <div className="order-1 lg:order-2 w-full max-w-sm lg:max-w-lg lg:w-1/2 flex justify-center z-10 lg:translate-x-7">
            <HeroImages />
          </div>

          {/* Left Text & Buttons */}
          <div className="order-2 lg:order-1 flex flex-col justify-center space-y-4 lg:space-y-6 z-20 lg:-translate-y-8 lg:-translate-x-12 max-w-xl text-center lg:text-left">
            <h1 className="font-headline-lg text-3xl md:text-5xl lg:text-6xl text-gray-900 dark:text-white leading-tight font-bold flex flex-col">
